...
void SimSN76489::Reset()
{
    for (int chip = 0; chip < 2; ++chip)
    {
        for (int i = 0; i <= 3; i++)
        {
            ...
            SN76489[chip].Registers[2 * i] = 1; /* tone freq=1 */
            SN76489[chip].Registers[2 * i + 1] = 0xf; /* vol=off */
        }

        /* Initialise latched register index */
        SN76489[chip].LatchedRegister = 0;

        /* Initialise noise generator */
        SN76489[chip].NoiseFreq = 0x10;
    }
}
...
void SimSN76489::Write(int chip, Register reg, uint8_t data)
{
    ...
    if (data & 0x80)
    {
        ...
        SN76489[chip].LatchedRegister = index = (data >> 4) & 0x07;
    }
    else
    {
        /* restore latched register index */
        index = SN76489[chip].LatchedRegister;
    }
    ...
    case 4: /* Tone Channels frequency */
    {
        if (data & 0x80)
        {
            ...
            SN76489[chip].Registers[index] = (SN76489[chip].Registers[index] & 0x3f0) | (data & 0xf);
        }
        else
        {
            /* Data byte  %0 - dddddd */
            SN76489[chip].Registers[index] = (SN76489[chip].Registers[index] & 0x00f) | ((data & 0x3f) << 4);
        }

        /* zero frequency behaves the same as a value of 1 */
        if (SN76489[chip].Registers[index] == 0)
        {
            SN76489[chip].Registers[index] = 1;
        }
        break;
    }
    ...
    {
        data &= 0x0f;
        SN76489[chip].Registers[index] = data;
...
void SimSN76489::Convert(Frame& frame)
{
    for (int chip = 0; chip < 2; ++chip)
    {
        bool enable0 = (SN76489[chip].Registers[1] != 0x0F);
        bool enable1 = (SN76489[chip].Registers[3] != 0x0F);
        bool enable2 = (SN76489[chip].Registers[5] != 0x0F);
        bool enable3 = (SN76489[chip].Registers[7] != 0x0F && (SN76489[chip].Registers[6] & 0x04));
        ...
        if (enable0)
        {
            uint16_t period = ConvertPeriod(SN76489[chip].Registers[0] << 1);
        ...
        if (enable1)
        {
            uint16_t period = ConvertPeriod(SN76489[chip].Registers[2] << 1);
        ...
        if (enable2)
        {
            uint16_t period = ConvertPeriod(SN76489[chip].Registers[4] << 1);
        ...
        if (enable3)
        {
            int NoiseFreq = SN76489[chip].NoiseFreq;
            if (NoiseFreq == 0x80)
                NoiseFreq = SN76489[chip].Registers[2 * 2];
